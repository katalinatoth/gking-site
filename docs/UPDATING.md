...
2. PDFs and images go under `_site/static/files/` (PDFs) or
   `_site/static/images/` (one-off images), or alongside the page's
...
If you prefer the terminal, drop a PDF under `_automation/intake/` (in the right
...
PDF), `_automation/scripts/quick_add.py` scaffolds the file in one command.

| Content type           | Easiest (any browser)             | Terminal                                  | No PDF / metadata-only                              |
|------------------------|-----------------------------------|-------------------------------------------|------------------------------------------------------|
| Paper / article         | [Issue Form](#quick-add-issue-form) | `_automation/intake/<file>.pdf`                       | `python3 _automation/scripts/quick_add.py paper …`              |
| Talk / presentation     | [Issue Form](#quick-add-issue-form) | `_automation/intake/talk/<file>.pdf`                  | `python3 _automation/scripts/quick_add.py talk …`               |
| Book                    | [Issue Form](#quick-add-issue-form) | `_automation/intake/book/<file>.pdf`                  | `python3 _automation/scripts/quick_add.py book …`               |
| Software / R package    | _(no PDF flow — software is URL-based)_ | _(none)_                                  | `python3 _automation/scripts/quick_add.py software --github …`  |
| Patent                  | [Issue Form](#quick-add-issue-form) (then `/type patent`) | optional `_automation/intake/<file>.pdf` then `/type patent` | `python3 _automation/scripts/quick_add.py patent --source …`   |
...
3. Wait ~2 minutes. The bot:
   ...
   - runs the same auto-import that powers the `_automation/intake/` flow
...
Both flows (Issue Form and `_automation/intake/` folder upload) share the same
auto-import code, so the resulting PR description and slash-command
edit surface are identical.

### Quick add: paper / article

The fastest path. Drop a PDF in `_automation/intake/` and the bot looks up the
...
cp ~/Downloads/whatever.pdf _automation/intake/     # filename doesn't matter
git add _automation/intake/whatever.pdf
...
1. Open <https://github.com/iqss-research/gking-site/tree/main/_automation/intake>
   (or click into the `_automation/intake/` folder from the repo's main page).
...
mkdir -p _automation/intake/talk
cp ~/Downloads/slides.pdf _automation/intake/talk/
git add _automation/intake/talk/slides.pdf
git commit -m "Drop slides for auto-import"
git push -u origin add-my-talk
gh pr create --fill
```

**GitHub.com flow:** same as paper, but click into the `_automation/intake/talk/`
...
```bash
python3 _automation/scripts/quick_add.py talk \
...
Drop the book PDF (or a manuscript / chapter sample) in `_automation/intake/book/`.
...
mkdir -p _automation/intake/book
cp ~/Downloads/manuscript.pdf _automation/intake/book/
git add _automation/intake/book/manuscript.pdf
...
```bash
python3 _automation/scripts/quick_add.py book \
...
Software entries are URL-based, not PDF-based, so there's no `_automation/intake/`
flow. The helper script is the easy path:

```bash
python3 _automation/scripts/quick_add.py software \
...
```bash
python3 _automation/scripts/quick_add.py patent \
...
drop it in `_automation/intake/` (default flow), let the bot scaffold the page,
...
- `.github/workflows/intake-publication.yml` runs on every PDF added
  under `_automation/intake/`, `_automation/intake/talk/`, or `_automation/intake/book/` in an
...
- Detects the content kind (from the dropdown in the issue form, or
  from the subfolder under `_automation/intake/`: `_automation/intake/foo.pdf` = paper,
  `_automation/intake/talk/foo.pdf` = presentation, `_automation/intake/book/foo.pdf` = book).
...
- Posts a PR description (issue-form flow) or follow-up comment
  (legacy `_automation/intake/` flow) that includes the title / authors /
...
`_automation/scripts/quick_add.py` runs entirely locally too — no PR opened, just
...
- The PR-triggered bot **only** runs from a pull-request branch, never
  ...
  `main`, remove the file from `_automation/intake/` and re-do it on a branch.
...
> [Quick add](#quick-add) flow above (drop a PDF in `_automation/intake/` and let
> the bot handle this, or run `_automation/scripts/quick_add.py paper …`). The
...
Put the PDF at `_site/static/files/<slug>.pdf`. Use a short,
...
- `files/...` URLs (no leading `/`) point at `_site/static/files/`.
...
>
> ```bash
> python3 _automation/scripts/import_drupal_redirects.py --apply
> ```
...
`_automation/scripts/enable-auto-push.sh` was run on this clone
