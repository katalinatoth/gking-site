...
│   └── EditMe/ResearchAreas/        # topic landing pages
...
The Gary King site uses this layout. **Everything you can edit lives
under one top-level folder, `EditMe/`.** Inside `EditMe/` there's one
sub-folder per item in the site's main menu plus a `UI/` folder for
the visual / aesthetic settings (CSS, page templates, per-section
layout overrides). Hugo Modules' `module.mounts` block in `hugo.yaml`
remaps every sub-folder of `EditMe/` onto Hugo's expected target
paths (`content/publication/…`, `layouts/_default/…`, `data/…`) at
build time, so URLs and the build are unchanged. The mounts block is
auto-generated by `_automation/scripts/generate_mounts.py`.

```
hugo-site/
├── EditMe/                                       # EVERY editable thing
│   ├── UI/                                       # CSS, layouts, per-section overrides
│   ├── HomePage/, Bio/, ResearchAreas/, Software/, Dataverse/, People/{ResearchGroup,Profiles,Authors}/, Teaching/, Blog/, Contact/, Misc/
│   ├── Writings/                                 # /publication/ + /talk/
│   │   ├── Articles/<Topic>/<Decade>/<slug>/     # ~270 journal articles & working papers
│   │   ├── Books/<Decade>/<slug>/
│   │   ├── Reports/<Decade>/<slug>/
│   │   ├── Patents/<Decade>/<slug>/
│   │   ├── CourtBriefs/<Decade>/<slug>/
│   │   ├── SoftwareNotes/<Decade>/<slug>/        # papers describing a software pkg
│   │   ├── Presentations/<title-slug>/<venue-slug>/   # talks
...
│       ├── writings/{intake_publication,fill_publication_from_doi,…}.py
│       ├── people/{rescrape_people,sync_research_group,…}.py
│       └── (top-level cross-section helpers)
├── docs/                                         # repo docs
│
├── .github/workflows/, .githooks/                # CI + auto-push hook (root-level)
├── _vendor/, go.mod, go.sum
└── hugo.yaml                                     # site config + auto-generated module.mounts block
```

**Why an `EditMe/` umbrella scales even better than per-section.**
With ~300 papers, ~350 people profiles, ~80 software pages, and ~10
different categories of helper scripts, the flat layout (2a) ends up
with `content/`, `data/`, `scripts/`, and `layouts/` each holding
hundreds of unrelated items, and a per-section variant of (2b) leaves
new contributors guessing whether to look in `writings/`, `talks/`,
or `_automation/`. The `EditMe/` umbrella collapses both axes: there
is **one place** to look ("everything I might want to edit lives
under `EditMe/`") and **one menu-driven sub-folder per editable
concept** (`Bio/`, `Writings/`, `Software/`, …). Within
`EditMe/Writings/`, papers nest by `<Type>/<Topic>/<Decade>/<slug>/`
and talks by `Presentations/<title-slug>/<venue-slug>/` so even the
biggest section stays browsable.

**Mounts are transparent for content URLs.** Permalinks like
`publication: /publication/:slug/` are driven by the **target** in
`module.mounts`, not by where files live on disk. So
`EditMe/Writings/Articles/CausalInference/2010s/foo/` mounted onto
`content/publication/foo` still serves at `/publication/foo/`. Page
bundles (an `index.md` next to its `featured.png` and supplementary
PDFs) move as a unit.
...
(`EditMe/UI/PerSectionLayouts/Writings/single.html`, etc.) **do**
move into the umbrella because Hugo's normal layout-lookup chain *is*
mount-aware. Only the shared theme bits above need to stay at the
root. See `EditMe/UI/PINNED-AT-ROOT.md` for the full list of
root-pinned folders and the exact fileExists / os.ReadDir calls that
force them.
...
```
EditMe/People/Authors/jane-doe/_index.md
```
...
(`EditMe/People/Profiles/<slug>/index.md`) from the Hugo *authors
taxonomy* (`EditMe/People/Authors/`). The taxonomy folder is for the
...
This pattern is in `EditMe/UI/PerSectionLayouts/Writings/list.html`
(in the EditMe/ variant of layout 2b) or `layouts/publication/list.html`
(in layout 2a). It is fast (<2k items on academic sites), works with
search engines (HTML present at load), and lets non-trivial filtering
happen without a framework.
...
1. Scrape the legacy categories from GTM / sitemap into a JSON map:
   `EditMe/Writings/Data/writings_legacy_map.json = { entries: { slug: { tab, drupal } } }`.
...
- **A way back up.** Breadcrumbs (Home / Writings / Paper Title) or a
  ...
  the only exit.
- **Related work.** Every paper/talk/EditMe/Software/Dataset has an automatic
...
- **Migration metadata lives in section data folders
  (`EditMe/Writings/Data/`, `EditMe/People/Data/`, …), never copied into content
...
- **Two mechanisms for everything else.** For arbitrary short URLs
  ...
  (`EditMe/Redirects/Data/redirects.yaml` in layout 2b,
...
- **A broken link from the outside is the owner's pain.** When you spot
  one, add an entry to `EditMe/Redirects/Data/redirects.yaml` (layout 2b) or
...
3. A list of any unique content types beyond paper/talk/EditMe/Software/Dataset.
