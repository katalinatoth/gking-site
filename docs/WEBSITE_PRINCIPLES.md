...
There are two ways to lay out an academic-Hugo repo. Pick one
deliberately and stick with it; mixing them is misery.

### 2a. Default Hugo layout (simplest; fine for small sites)
...
├── _vendor/, go.mod, go.sum   # hugo mod vendor output (checked in)
├── hugo.yaml                  # site config
├── .github/workflows/deploy.yml
└── docs/{UPDATING.md, …}      # repo docs
```

This is the layout Hugo expects out of the box and is what you should
start with. Once the site outgrows ~200 pieces of content (so that
"add a paper" touches files in 3+ unrelated top-level folders —
`content/`, `data/`, `static/files/`, `scripts/`), graduate to (2b).

### 2b. Section-driven layout (recommended once the site has scale)

The Gary King site uses this layout. Top-level folders mirror the
website's nav (Bio, Writings, People, Software, …) and each one
bundles its own content + layouts + data + scripts. Hugo Modules'
`module.mounts` block in `hugo.yaml` remaps every section folder onto
Hugo's expected target paths (`content/publication/…`,
`layouts/_default/…`, `data/…`) at build time, so URLs and the build
are unchanged.

```
hugo-site/
├── home/, bio/, teaching/, contact/, blog/, research-areas/, dataverse/, pages/
│       └── content/, layouts/                    # per-section
├── writings/                                     # /publication/
│       ├── content/<slug>/index.md               # ~1 folder per paper
│       ├── data/{featured_publications.yaml, writings_legacy_map.json, …}
│       ├── layouts/{list,single}.html
│       └── scripts/{intake_publication,fill_from_doi,…}.py
├── talks/, software/                             # parallel structure
├── people/                                       # /research-group/, /people/, /authors/
│       ├── content/{group,profiles,authors}/<slug>/index.md
│       ├── data/research_group.json
│       ├── layouts/{people,research-group}/single.html
│       └── scripts/{rescrape_people,sync_research_group,…}.py
├── redirects/                                    # legacy URL aliases
│       ├── content/                              # auto-generated stubs (gitignored)
│       └── data/redirects.yaml
│
├── layouts/                                      # SHARED theme bits — must
│       ├── baseof.html, alias.html               # stay at the project root
│       ├── _default/, _partials/, shortcodes/    # because HugoBlox's
│       └── page/                                 # get_hook partial reads
├── assets/css/custom.css                         # this and `assets/css/` with
│                                                 # os.ReadDir / fileExists,
│                                                 # which are NOT mount-aware.
│
├── _site/                                        # cross-section Hugo plumbing
│       ├── static/files/, static/images/
│       ├── archetypes/, i18n/
│       └── data/                                 # site-wide data outputs
├── _automation/                                  # maintenance + intake
│       ├── intake/{,talk,book}/                  # drop-zone for the auto-import bot
│       └── scripts/                              # cross-section helpers
├── docs/                                         # repo docs
│
├── .github/workflows/, .githooks/                # CI + auto-push hook (root-level)
├── _vendor/, go.mod, go.sum
└── hugo.yaml                                     # site config + module.mounts block
```

**Why section-driven scales better.** With ~300 papers, ~350 people
profiles, ~80 software pages, and ~10 different categories of helper
scripts, the flat layout (2a) ends up with `content/`, `data/`,
`scripts/`, and `layouts/` each holding hundreds of unrelated items.
Adding a feature like "intake bot for new papers" touches all four.
With (2b), everything Writings-related — content, layout, data,
intake script — lives under `writings/`, and you can `git log` /
`git grep` the section in isolation.

**Mounts are transparent for content URLs.** Permalinks like
`publication: /publication/:slug/` are driven by the **target** in
`module.mounts`, not by where files live on disk. So `writings/content/foo`
mounted onto `content/publication/foo` still serves at
`/publication/foo/`. Page bundles (an `index.md` next to its
`featured.png` and supplementary PDFs) move as a unit.

**Three folders MUST stay at the project root** because they are read
by tools that don't follow Hugo mounts:

- `layouts/` (the shared bits — `baseof.html`, `_default/`,
  `_partials/`, `shortcodes/`, `page/`). HugoBlox's
  `_partials/functions/get_hook.html` calls
  `os.ReadDir "layouts/_partials/hooks/body-end/"` to discover hook
  scripts. `os.ReadDir` does a literal filesystem read.
- `assets/`. HugoBlox's tailwind theme partial
  (`_vendor/.../site_head.html`) calls
  `fileExists "assets/css/custom.css"`. Same reason.
- `.github/` and `.githooks/`. GitHub Actions and git both look for
  these by literal path.

Section-specific layouts (`writings/layouts/single.html`, etc.) **do**
move into their section folders because Hugo's normal layout-lookup
chain *is* mount-aware. Only the shared theme bits above need to stay
at the root.

**Rule of thumb for overrides:** if you need to customize a Blox
partial, copy it from `_vendor/.../layouts/_partials/<path>` to
`layouts/_partials/<path>` (the project root, NOT under a section)
and edit there. Hugo picks the project copy.
...
- `links` with `name` + `url` renders as styled buttons. `url` starting
  with `files/…` points at the static-files mount (physically
  `_site/static/files/` in layout 2b, `static/files/` in layout 2a).
...
```
people/content/authors/jane-doe/_index.md
```

Contains `user_groups` to slot them into Research Group sections
(`Current Members`, `Alumni`, …). Avatar is `avatar.jpg` alongside.

The Gary King site additionally separates *roster* profiles
(`people/content/profiles/<slug>/index.md`) from the Hugo *authors
taxonomy* (`people/content/authors/`). The taxonomy folder is for the
small number of people who are listed as authors on publications;
the much larger profiles tree is the filterable People page roster.
...
This pattern is in `writings/layouts/list.html` (in layout 2b) or
`layouts/publication/list.html` (in layout 2a). It is fast (<2k items
...
1. Scrape the legacy categories from GTM / sitemap into a JSON map:
   `writings/data/writings_legacy_map.json = { entries: { slug: { tab, drupal } } }`.
...
in one regenerable JSON file under the section's `data/` folder.
...
- **Migration metadata lives in section data folders
  (`writings/data/`, `people/data/`, …), never copied into content
  front matter.** (§7.) Legacy category, legacy Drupal node ID,
  thumbnail URL are attached by slug at build time.
- **Button labels are centralized in `i18n/en.yaml`** (or
  `_site/i18n/en.yaml` in layout 2b). Never inline the string
  "Article" or "Publisher's Version" in a template.
...
- **Two mechanisms for everything else.** For arbitrary short URLs
  ...
  paper), maintain one centralized data file
  (`redirects/data/redirects.yaml` in layout 2b,
  `data/redirects.yaml` in layout 2a) that non-technical owners can
  edit on GitHub. A small pre-build
...
- **A broken link from the outside is the owner's pain.** When you spot
  one, add an entry to `redirects/data/redirects.yaml` (layout 2b) or
  `data/redirects.yaml` (layout 2a); don't tell people to update
  their bookmarks.
...
- **All PDFs live in the static-files mount** (physically
  `_site/static/files/` in layout 2b, `static/files/` in layout 2a).
  Never link to someone else's copy of the author's own paper; host
  it here.
...
- **If no owned image exists, pull from the public web and commit it
  ...
  1. The owner's institutional profile page (the university almost
     always has usage rights; mirror the photo into `assets/media/`
     or `_site/static/images/` in layout 2b — `assets/images/` or
     `static/images/` in layout 2a).
...
- **Record provenance.** Every non-owned image gets a
  `_credits.md` line in the section's `data/image_credits.yaml` (or
  a top-level `data/image_credits.yaml` in layout 2a), or a
  `credit:` entry in the page's front matter, listing the source
  URL, date fetched, and licence ("CC-BY 4.0, Wikimedia",
  "institutional profile, fair use", "public press kit, Harvard
  Gazette").
