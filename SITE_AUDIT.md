...
- **Images** are converted to WebP by Hugo's image pipeline
  ...
  raster assets are a handful of JPGs in `static/images/`.
...
`data/research_areas.json` would be a 30-minute job and give
the site five new "shop window" pages.
...
6. **Search result pinning.** The Pagefind-powered site search
   ...
   `data/pinned_searches.yaml` so typing "matching" or "ei"
...
8. **Paper series / co-author graph landing page.** Given all
   ...
   `content/authors/`, a small D3 or sigma.js graph on
