...
Usage:
    python3 _automation/scripts/import_drupal_redirects.py --apply  # writes
    python3 _automation/scripts/import_drupal_redirects.py          # dry-run
    python3 _automation/scripts/import_drupal_redirects.py --diff   # diff only
"""
...
# Hugo `target` -> physical source folder. Mirrors the mounts in
# `hugo.yaml`: redirect collision detection has to look at the actual
...
_NEW_SOFTWARE_DIRS = sorted(
    d.name for d in _SECTION_TO_PATH["software"].iterdir()
    if d.is_dir() and not d.name.startswith("_")
) if _SECTION_TO_PATH["software"].is_dir() else []
...
def _section_dir(name: str) -> Path:
    """Resolve a Hugo section name (e.g. `publication`) to its physical
    source folder under the section-driven repo layout."""
    return _SECTION_TO_PATH.get(name, ROOT / name)
...
def _conflicts_with_existing(path: str) -> bool:
    ...
    on the new site. Checks `_site/static/` (raw assets) and every
    section's `<section>/content/` source folder.
    """
    rel = path.strip("/")
    if not rel:
        return False
    static_match = ROOT / "_site" / "static" / rel
    if static_match.exists():
        return True
    head = rel.split("/", 1)[0]
    rest = rel.split("/", 1)[1] if "/" in rel else None
    section_dir = _SECTION_TO_PATH.get(head)
    if section_dir is not None:
        if rest is None:
            return section_dir.is_dir()
...
def _resolve_target_inner(raw: str) -> tuple[str, str | None]:
    ...
    if file_match:
        ...
        candidate_static = ROOT / "_site" / "static" / "files" / fname
        ...
        if base.startswith("/sites/"):
            return (f"https://gking.harvard.edu{base}", "file not in new _site/static/files/ — preserved as absolute archived URL")
        return (f"https://gking.harvard.edu{base}", "file not in new _site/static/files/ — preserved as absolute archived URL")
...
def render_block(rows: list[tuple[str, str, int, str | None, str]]) -> str:
    ...
    lines: list[str] = [
        ...
        "# Re-generate with: python3 _automation/scripts/import_drupal_redirects.py --apply",
        "#",
        ...
        "# `from` column. The resolver in _automation/scripts/import_drupal_redirects.py",
...
def render_vanity_block(rows: list[tuple[str, str, int, str | None, str]]) -> str:
    lines: list[str] = [
        ...
        "#         _automation/scripts/import_drupal_redirects.py, derived from",
