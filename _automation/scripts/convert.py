...
HUGO_DIR = Path(__file__).resolve().parents[2]  # the gking-site checkout root
...
# In the section-driven layout, content lives under <section>/content/ rather
...
def section_dir(section: str) -> Path:
    """Return the physical folder for a logical Hugo section name."""
    if section in _SECTION_DIRS:
        return _SECTION_DIRS[section]
    raise KeyError(
        f"convert.py: unknown section {section!r}; update _SECTION_DIRS in "
        f"_automation/scripts/convert.py to map it to its <section>/content/ path."
    )
...
def convert_publications(pdf_mapping):
    ...
    for pub in pubs:
        ...
        pub_dir = section_dir(section) / slug
...
def convert_software():
    ...
    for sw in software:
        ...
        sw_dir = section_dir("software") / slug
...
def convert_home(pdf_mapping):
    ...
    content = "\n".join(lines)
    with open(home_dir / "_index.md", "w", encoding="utf-8") as f:
        f.write(content)
...
def copy_pdfs():
    ...
    print(f"  Copied {count} files to _site/static/files/")
...
def create_author_page():
    ...
    author_dir = section_dir("authors") / "gary-king"
