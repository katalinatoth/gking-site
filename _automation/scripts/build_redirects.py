#!/usr/bin/env python3
"""Generate Hugo content stubs from redirects/data/redirects.yaml.

For every entry `{ from: X, to: Y }` this writes a small content page at
`redirects/content/X/index.md` with `url: /X/` and a `redirect` layout.
Hugo then renders that page as a meta-refresh HTML file pointing to `Y`
(via the mount `redirects/content -> content/_redirects`).

The `redirects/content/` directory is ignored by git and regenerated on
every build. Gary edits ONLY `redirects/data/redirects.yaml`; the CI
...
OUT_DIR = ROOT / "redirects" / "content"
...
def existing_top_level_slugs() -> set[str]:
    ...
    The check unions:
      - subfolders of `home/content/` and `pages/content/` (which mount to
        the URL root), plus
...
def write_netlify_redirects(rules: list[tuple[str, str, int]]) -> None:
    ...
    lines = [
        ...
        "# from redirects/data/redirects.yaml.",
        "# Honoured by Netlify, Cloudflare Pages, etc. Ignored by GitHub Pages,",
        "# which serves the meta-refresh stubs in redirects/content/ instead.",
        "",
    ]
...
def ensure_intermediate_index(parent: Path) -> None:
    """Make sure every intermediate directory under redirects/content/
...
"""
    ...
    for entry in entries:
        ...
        if path in seen:
            die(f"Duplicate `from: /{path}/` in redirects/data/redirects.yaml.")
        ...
        # redirects/content/<segs>/index.md and an absolute `url:`).
