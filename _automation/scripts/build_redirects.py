#!/usr/bin/env python3
"""Generate Hugo content stubs from EditMe/Redirects/Data/redirects.yaml.

For every entry `{ from: X, to: Y }` this writes a small content page at
`EditMe/Redirects/content/X/index.md` with `url: /X/` and a `redirect` layout.
Hugo then renders that page as a meta-refresh HTML file pointing to `Y`
(via the mount `EditMe/Redirects/content -> content/_redirects`).

The `EditMe/Redirects/content/` directory is ignored by git and regenerated on
every build. Gary edits ONLY `EditMe/Redirects/Data/redirects.yaml`; the CI
workflow runs this script before the Hugo build.
"""
...
# Repo layout (post-reorg): this file lives at
# `_automation/scripts/build_redirects.py`, so the repo root is two levels up.
ROOT = Path(__file__).resolve().parents[2]
DATA_FILE = ROOT / "EditMe" / "Redirects" / "Data" / "redirects.yaml"
OUT_DIR = ROOT / "EditMe" / "Redirects" / "content"
NETLIFY_REDIRECTS_FILE = ROOT / "_site" / "static" / "_redirects"

# The folders whose top-level entries occupy the URL space at `/<seg>/`.
# Used by `existing_top_level_slugs()` to flag short-URL collisions.
TOP_LEVEL_URL_SOURCES: tuple[Path, ...] = (
    ROOT / "EditMe" / "HomePage",
    ROOT / "EditMe" / "Misc",
)
# Plus these mount targets (canonical Hugo section names from hugo.yaml).
TOP_LEVEL_URL_SECTIONS: tuple[str, ...] = (
    "bio",
    "blog",
    "contact",
    "dataverse",
...
def existing_top_level_slugs() -> set[str]:
    """Slugs currently owned by something else at the URL root.

    We only guard against direct collisions where a redirect's *single-segment*
    `from` would shadow an existing top-level page (e.g. /bio/, /apply/).
    Multi-segment redirect paths and section-internal paths like
    /publication/foo/ are fine.

    The check unions:
      - subfolders of `EditMe/HomePage/` and `EditMe/Misc/` (which mount to
        the URL root), plus
      - the canonical section names from `hugo.yaml` mounts.
    """
    slugs: set[str] = set(TOP_LEVEL_URL_SECTIONS)
    for source in TOP_LEVEL_URL_SOURCES:
        if not source.exists():
            continue
        for entry in source.iterdir():
            if entry.name.startswith("_"):
                continue
            # README.md / readme.md sit alongside content but are explicitly
            # excluded from the Hugo build via `ignoreFiles` in hugo.yaml,
            # so they don't actually own a URL.
...
def write_netlify_redirects(rules: list[tuple[str, str, int]]) -> None:
    ...
    if not rules:
        ...
        return
    lines = [
        "# Auto-generated by _automation/scripts/build_redirects.py",
...
def ensure_intermediate_index(parent: Path) -> None:
    """Make sure every intermediate directory under EditMe/Redirects/content/
...
"""
    ...
    for entry in entries:
        ...
        if path in seen:
            die(f"Duplicate `from: /{path}/` in EditMe/Redirects/Data/redirects.yaml.")
        ...
        # EditMe/Redirects/content/<segs>/index.md and an absolute `url:`).
        first_seg = path.split("/", 1)[0].lower()
        if "/" not in path and first_seg in existing:
            die(
                f"Entry `from: /{path}/` collides with an existing top-level "
                f"page `/{first_seg}/` — pick a different short URL."
            )
        seen.add(path)
