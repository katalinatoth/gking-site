...
def index_titles(section: str) -> dict[str, str]:
...
def main() -> int:
    ...
    actual_pub = {d.name for d in (CONTENT / "publication").iterdir() if d.is_dir()}
    actual_talk = {d.name for d in (CONTENT / "talk").iterdir() if d.is_dir()}
    ...
    titles_pub = index_titles("publication")
    titles_talk = index_titles("talk")
