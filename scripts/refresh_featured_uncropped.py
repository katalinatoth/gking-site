...
Pipeline per content/publication/<slug>/:
  ...
  2. Fallback when no scraped HTML / no usable URL / download fails: open
     the linked PDF in static/files/<name>.pdf, enumerate every embedded
     raster image with PyMuPDF, perceptual-hash compare each against the
     CURRENT cropped featured.png, and save the closest match (within a
     Hamming-distance threshold) as featured.<ext>.
...
Usage:
  python3 scripts/refresh_featured_uncropped.py             # dry run
  python3 scripts/refresh_featured_uncropped.py --apply     # do it
  python3 scripts/refresh_featured_uncropped.py --only foo  # one slug
  python3 scripts/refresh_featured_uncropped.py --apply --force

Requires:
  pip install -r scripts/requirements-featured.txt
"""
...
def find_pdf_path(d: Path) -> Path | None:
    ...
    if not m:
        return None
    pdf = ROOT / "static" / "files" / m.group(1)
