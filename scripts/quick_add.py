...
Companion to scripts/intake_publication.py. The intake bot is great for
papers/talks/books that come as a PDF — drop the file under intake/ and
the bot fills in the front matter from Crossref. But:
...
This script handles all five content types via a uniform CLI:
    ...
    python3 scripts/quick_add.py software --slug my-tool \\
        --title "MyTool: An R Package for ..." \\
        --year 2026 \\
        --authors "Gary King; Jane Doe" \\
        --github https://github.com/IQSS/my-tool

    python3 scripts/quick_add.py patent --slug my-patent-2025 \\
        --title "..." \\
        --year 2025 \\
    ...
    python3 scripts/quick_add.py paper --slug my-paper \\
        --title "..." --year 2026 --authors "Gary King" \\
        [--pdf ~/Downloads/my-paper.pdf]
        [--doi 10.1017/...]

    python3 scripts/quick_add.py talk --slug my-talk \\
        --title "..." --date 2026-04-30 --authors "Gary King" \\
        [--pdf ~/Downloads/slides.pdf]

    python3 scripts/quick_add.py book --slug my-book \\
        --title "..." --year 2026 --authors "Gary King" \\
...
minutes. With auto-push enabled (scripts/enable-auto-push.sh) one
`git commit` is all you need.
"""
...
def _section_dir(kind: str) -> Path:
    """All non-talk content lives under content/publication/."""
    if kind == "talk":
        return ROOT / "content" / "talk"
    return ROOT / "content" / "publication"
...
def _maybe_copy_pdf(src: Path | None, slug: str, dry_run: bool) -> Path | None:
    """Copy a user-supplied PDF into static/files/<slug>.pdf and return
    ...
    dest = ROOT / "static" / "files" / f"{slug}.pdf"
    if dry_run:
        return dest
...
def _append_software_row(slug: str, year: int, status: str, dry_run: bool) -> Path:
    """Append a row to data/software_legacy_rows.yaml.
    ...
    """
    path = ROOT / "data" / "software_legacy_rows.yaml"
...
def _add_common(p: argparse.ArgumentParser) -> None:
    ...
    p.add_argument("--pdf", help="Optional local PDF; copied to static/files/.")
    p.add_argument(
        "--dry-run",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="quick_add",
        description=(
            "Scaffold a content/<section>/<slug>/index.md and update "
            "data/writings_legacy_map.json (and data/software_legacy_rows.yaml "
            "for software). One subcommand per content type."
        ),
