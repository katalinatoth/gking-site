...
opens an issue from the **Upload a paper** template. The workflow runs:
    ...
    python3 scripts/intake_from_issue.py \
...
3.  Drops the main PDF into `intake/`, `intake/talk/`, or
    ...
    to `static/files/<slug>.pdf`, and the writings legacy map all
    happen via the same code path as the legacy intake/ flow.
4.  Saves any supplementary PDFs straight into `static/files/` with
    deterministic names (`<slug>-supplement-<N>.pdf`) and patches the
    new index.md to add a `links:` entry for each one.
5.  Patches `aliases:` if a short URL was set.
6.  Renders every numbered figure from the PDF into
    `content/publication/<slug>/_intake_figures/` (or
    `content/talk/<slug>/_intake_figures/`).
...
# package or fiddling with PYTHONPATH.
_intake_spec = importlib.util.spec_from_file_location(
    "intake_publication", THIS_DIR / "intake_publication.py"
)
...
_figs_spec = importlib.util.spec_from_file_location(
    "extract_all_figures", THIS_DIR / "extract_all_figures.py"
)
...
def main() -> int:
    ...
    # 1) Run the existing pipeline. This moves target -> static/files/<slug>.pdf
    #    and writes content/publication/<slug>/index.md (or content/talk/<slug>/).
